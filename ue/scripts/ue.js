...
function fixAutoLinkedBlocks() {
  const linkBlocks = [
    { pattern: '/fragments/', name: 'fragment', label: 'Fragment' },
    { pattern: '/schedules/', name: 'schedule', label: 'Schedule' },
    { pattern: 'youtube.com', name: 'youtube', label: 'YouTube' },
    { pattern: 'youtu.be', name: 'youtube', label: 'YouTube' },
  ];

  // Restore hrefs that were hidden from ak.js and convert to block components
  ...
  // Convert richtext wrappers containing these links to block components
  document.querySelectorAll('[data-aue-type="richtext"][data-aue-behavior="component"]').forEach((el) => {
    const link = el.querySelector('a');
    if (!link) return;
    const href = link.getAttribute('href') || '';

    const match = linkBlocks.find((lb) => href.includes(lb.pattern));
    if (!match) return;

    el.setAttribute('data-aue-type', 'component');
    el.setAttribute('data-aue-label', match.label);
    el.setAttribute('data-aue-component', match.name);
    el.setAttribute('data-aue-model', match.name);
    el.setAttribute('data-block-name', match.name);
    el.classList.remove('default-content');
    el.classList.add(match.name);
  });
}
